...
impl From<&IcmpTypeV4> for u8 {
  fn from(type_: &IcmpTypeV4) -> Self {
    (*type_) as Self
  }
}

#[test]
fn test_icmp_type_v4_to_u8() {
...
#[test]
fn test_u8_to_icmp_type_v4() {
  assert_eq!(EchoReply, IcmpTypeV4::from(0));
  for i in 1..=2 {
    assert_eq!(_Unassigned1To2, IcmpTypeV4::from(i));
  }
  assert_eq!(DestinationUnreachable, IcmpTypeV4::from(3));
  assert_eq!(_InformationReply, IcmpTypeV4::from(16));
  for i in 19..=29 {
    assert_eq!(_Reserved19To29, IcmpTypeV4::from(i));
  }
  assert_eq!(_Reserved255, IcmpTypeV4::from(255));
}